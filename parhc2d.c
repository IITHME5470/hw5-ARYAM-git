...
void get_rhs(int nx, int nxglob, int ny, int nyglob, int istglob, int ienglob, int jstglob, int jenglob, double dx, double dy, double *xleftghost, double *xrightghost, double *ybotghost, double *ytopghost, double kdiff, double *x, double *y, double **T, double **rhs)
{
  ...
  if(jstglob==0)  //processors adjacent to the bottom end of the domain
    ...
    for(i=1; i<nx-1; i++)
     rhs[i][j] = kdiff*(T[i+1][j] + T[i-1][j] - 2.0*T[i][j])/dxsq +
                kdiff*(ybotghost[i] + T[i][j+1] - 2.0*T[i][j])/dysq;
  ...
  i = nx-1; j = ny-1;
    if(ienglob==nxglob-1 || jenglob==nyglob-1)
      ...
      rhs[i][j] = kdiff*(T[i-1][j] + xrightghost[j] - 2.0*T[i][j])/dxsq +
                  kdiff*(T[i][j-1] + ytopghost[i] - 2.0*T[i][j])/dysq;
...
void halo_exchange_2d_x(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *xleftghost, double *xrightghost, double *sendbuf_x, double *recvbuf_x)
{
  ...
  // send and recv
  MPI_Recv(recvbuf_x, ny, MPI_DOUBLE, left_nb, 10, MPI_COMM_WORLD, &status);
  MPI_Send(sendbuf_x, ny, MPI_DOUBLE, right_nb, 10, MPI_COMM_WORLD);
  // unpack recv buffer
  for(j=0; j<ny; j++)
...
void halo_exchange_2d_y(int rank, int rank_x, int rank_y, int size, int px, int py, int nx, int ny, int nxglob, int nyglob, double *x, double *y, double **T, double *ybotghost, double *ytopghost, double *sendbuf_y, double *recvbuf_y)
{
  ...
  for(i=0; i<nx; i++)
    sendbuf_y[i] = T[i][0];
  // send and recv
  MPI_Recv(recvbuf_y, nx, MPI_DOUBLE, top_nb, 20, MPI_COMM_WORLD, &status);
  MPI_Send(sendbuf_y, nx, MPI_DOUBLE, bot_nb, 20, MPI_COMM_WORLD);
  // unpack recv buffer
  for(i=0; i<nx; i++)
  ...
  // send and recv
  MPI_Recv(recvbuf_y, nx, MPI_DOUBLE, bot_nb, 30, MPI_COMM_WORLD, &status);
  MPI_Send(sendbuf_y, nx, MPI_DOUBLE, top_nb, 30, MPI_COMM_WORLD);
  // unpack recv buffer
  for(i=0; i<nx; i++)
...
// output_soln(nx,ny,it,tcurr,x,y,T);
  ...
  if (rank == 0) {
    double avg_time = total_time / num_time_steps;
    printf("Average time per timestep: %lf seconds\n", avg_time);
  }
 for(i=0; i<nx; i++)
   free(T[i]);
 free(T);
